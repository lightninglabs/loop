...
// validOpTrueProof creates a structurally valid OP_TRUE proof with the given
// asset identity, amount, and deterministic anchor key.
...
func TestCreateOpTrueSweepVpktMultipleInputs(t *testing.T) {
	...
	tests := []struct {
		...
		packetVersion  tappsbt.VPacketVersion
	}{
		{
			name: "v0 address", addressVersion: address.V0,
			packetVersion: tappsbt.V0,
		},
		{
			name: "v1 address", addressVersion: address.V1,
			packetVersion: tappsbt.V1,
		},
	}
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			addr := addr
			addr.Version = testCase.addressVersion
			packet, err := CreateOpTrueSweepVpkt(
				t.Context(), proofs, &addr,
			)
			require.NoError(t, err)
			require.Equal(t, testCase.packetVersion, packet.Version)
			require.Len(t, packet.Outputs, 2)
			require.True(t, packet.Outputs[0].Type.IsSplitRoot())
			require.False(t, packet.Outputs[0].Interactive)
			require.False(t, packet.Outputs[1].Interactive)
			require.Equal(t, uint32(1),
				packet.Outputs[1].AnchorOutputIndex)
			require.Same(t, &addr, packet.Outputs[1].Address)
