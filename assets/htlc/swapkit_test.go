...
// scalarKey derives a deterministic test key from a one-byte scalar.
...
// vectorParams returns the immutable inputs used by the legacy contract
// vectors.
...
// newVectorKit creates a swap kit from the legacy contract vector inputs.
...
// assertScriptSpend executes one HTLC script path and checks its result.
...
// newWitnessFixture creates a proof-bound HTLC and sweep packet using the
// default asset and proof versions.
...
// newWitnessFixtureWithVersions creates a proof-bound HTLC and sweep packet
// using explicit asset and proof versions.
