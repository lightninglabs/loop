...
// sending. Its hash must match the validated prepay invoice, not the probe.
...
type Payments interface {
	...
	// Probe checks whether an invoice's destination can be reached without
	// paying the invoice. It returns the outcome and estimated routing fee
	// in millisatoshis. A nil error does not imply a successful outcome, and
	// success does not guarantee that a later payment will succeed.
	Probe(context.Context, string) (ProbeOutcome, uint64, error)
