...
type Terms struct {
	...
	// Fee is the server's total charge in indivisible asset units, including
	// its estimated funding cost. The client pays it in full when buying
	// the reservation. Execution owes Amount + Fee - prepaid credit, so
	// crediting the prepay prevents double charging; it is not a refund of
	// the funding charge. The server also keeps the prepay if the reservation
	// is unused.
	//
	...
	// client-side sweep fees are separate. The server pays its funding
	// transaction's miner fee from its own BTC wallet.
	//
