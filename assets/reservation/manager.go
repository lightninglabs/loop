...
func (m *Manager) Run(ctx context.Context) error {
	...
	// TODO(hieblmi: read non-final reservations only.
	readCtx, readCancel := context.WithTimeout(runCtx, m.cfg.CallTimeout)
	reservations, err := m.cfg.FSM.Store.GetReservations(readCtx)
	readCancel()
	if err != nil {
		return err
	}
	...
	for {
		...
		case request := <-m.requests:
			...
			// event; approval, cancellation, and probe commands go below.
			if request.create {
...
// RetryProbes asks the worker to probe the same unapproved quote again. The FSM
// clears the saved skip preference before probing and retains the quoted terms.
// The request is rejected once the purchase has left AwaitApproval.
func (m *Manager) RetryProbes(ctx context.Context,
...
// Cancel asks the worker to resolve the prepay and cancel an unpaid purchase.
// A settled payment continues toward delivery. If the worker has finished or
// its current state rejects cancellation, return the saved progress instead.
// A successful call may still leave cancellation in progress.
func (m *Manager) Cancel(ctx context.Context, id ID) (*Reservation, error) {
	r, err := m.submit(&managerRequest{
		ctx:   ctx,
		id:    id,
		event: OnCancel,
	})
	if errors.Is(err, ErrNotFound) || errors.Is(err, fsm.ErrEventRejected) {
		// A repeated cancel returns the saved outcome, including payment
		// that settled before cancellation reached the worker.
		return m.Get(ctx, id)
	}
	return r, err
}
...
// List loads all saved reservations within CallTimeout, including final ones.
// It reads storage directly, independently of the live worker registry.
func (m *Manager) List(ctx context.Context) ([]*Reservation, error) {
	readCtx, cancel := context.WithTimeout(ctx, m.cfg.CallTimeout)
	defer cancel()
	return m.cfg.FSM.Store.GetReservations(readCtx)
}
