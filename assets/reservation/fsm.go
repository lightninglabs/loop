...
const (
	...
	OnProbeAgain fsm.EventType = "OnProbeAgain"
...
// stores the routing cap and SkipProbe, and a probe retry clears SkipProbe.
...
func (f *FSM) enterState(ctx context.Context, n fsm.Notification) {
	...
	r.State = n.NextState
	// An explicit retry enables probing, including after a restart that
	// interrupts this attempt. Save that choice with the state transition.
	if n.Event == OnProbeAgain {
		r.SkipProbe = false
	}
