...
func updatePurchase(ctx context.Context, q *sqlc.Queries,
	...
	if r.ProbeNodeKey != nil {
		args.ProbeNodeKey = r.ProbeNodeKey.SerializeCompressed()
	}
	if !r.ProbeDeadline.IsZero() {
		args.ProbeDeadline = sql.NullTime{Time: r.ProbeDeadline, Valid: true}
	}
	...
	for _, field := range []struct {
		...
		{r.ProbeRequest, &args.ProbeRequest},
		{r.ProbeResult, &args.ProbeResult},
		{
			r.Quote,
			&args.Quote,
		},
...
func readPurchase(row sqlc.AssetReservation, r *Reservation) error {
	if len(row.ProbeRequest) != 0 {
		var err error
		r.ProbeNodeKey, err = btcec.ParsePubKey(row.ProbeNodeKey)
		if err != nil {
			return err
		}
		r.ProbeRequest = &routerrpc.SendPaymentRequest{}
		if err := proto.Unmarshal(row.ProbeRequest, r.ProbeRequest); err != nil {
			return err
		}
	}
	if len(row.ProbeResult) != 0 {
		r.ProbeResult = &lnrpc.Payment{}
		if err := proto.Unmarshal(row.ProbeResult, r.ProbeResult); err != nil {
			return err
		}
	}
	if row.ProbeDeadline.Valid {
		r.ProbeDeadline = row.ProbeDeadline.Time.UTC()
	}
