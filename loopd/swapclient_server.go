...
func (s *swapClientServer) ListStaticAddressSwaps(_ context.Context,
	_ *looprpc.ListStaticAddressSwapsRequest) (
	*looprpc.ListStaticAddressSwapsResponse, error) {

	swaps, err := s.staticLoopInManager.GetAllSwaps()
	if err != nil {
		return nil, err
	}
	...
	for _, s := range swaps {
		swap := &looprpc.StaticAddressLoopInSwap{
			...
			State:            toClientStaticAddressLoopInState(s.GetState()),
			SwapAmount:       int64(s.TotalDepositAmount()),
		}
