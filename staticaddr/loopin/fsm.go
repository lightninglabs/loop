...
func (f *FSM) updateLoopIn(notification fsm.Notification) {
	...
	stored, err := f.cfg.Store.IsStored(f.ctx, f.loopIn.SwapHash)
	if err != nil {
...
func isUpdateSkipped(notification fsm.Notification,
	...
	// If we transitioned from the empty state to InitHtlc there's still no
	// loop-in persisted, so we don't need to update it.
	if prevState == fsm.EmptyState && l.IsInState(InitHtlc) {
		return true
	}
