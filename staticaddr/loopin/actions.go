...
func (f *FSM) InitHtlcAction(_ fsm.EventContext) fsm.EventType {
	...
	err = f.cfg.Store.CreateLoopIn(f.ctx, f.loopIn)
	if err != nil {
...
func (f *FSM) SignHtlcTxAction(_ fsm.EventContext) fsm.EventType {
	...
	defer f.cleanUpSessions(htlcSessions)
	...
	defer f.cleanUpSessions(htlcSessionsHighFee)
	...
	defer f.cleanUpSessions(htlcSessionsExtremelyHighFee)
...
func (f *FSM) cleanUpSessions(sessions []*input.MuSig2SessionInfo) {
	for _, s := range sessions {
		err := f.cfg.Signer.MuSig2Cleanup(f.ctx, s.SessionID)
		if err != nil {
...
func (f *FSM) MonitorInvoiceAndHtlcTxAction(_ fsm.EventContext) fsm.EventType {
	// Look up the state of the swap invoice. If upon restart recovery, we
	// land here and observe that the invoice is already canceled, it can
	...
	// paying the invoice.
	invoice, err := f.cfg.LndClient.LookupInvoice(f.ctx, f.loopIn.SwapHash)
	if err != nil {
		err = fmt.Errorf("unable to lookup invoice: %w", err)

		return f.HandleError(err)
	}

	var (
		invoiceUpdateChan = make(<-chan lndclient.InvoiceUpdate)
		invoiceErrChan    = make(<-chan error)
	)

	switch invoice.State {
	case invoices.ContractSettled:
		// If the invoice is already settled, we can proceed with the
		// sweepless sweep.
		return OnPaymentReceived

	case invoices.ContractCanceled:
		// If the invoice is already canceled a user-provided payment
		// timeout occurred previously, so we continue to monitor for
		// the server publishing the htlc tx without paying the invoice.

	default:
		// Subscribe to swap invoice settlement.
		subscribeInvoice := f.cfg.InvoicesClient.SubscribeSingleInvoice
		invoiceUpdateChan, invoiceErrChan, err = subscribeInvoice(
	...
	registerConfs := f.cfg.ChainNotifier.RegisterConfirmationsNtfn
	htlcConfChan, htlcErrConfChan, err := registerConfs(
	...
	blockChan, blockChanErr, err := registerBlocks(f.ctx)
	if err != nil {
	...
	for {
		...
		case currentHeight := <-blockChan:
			...
			if !htlcConfirmed {
				...
				// be failed. We cancelled the invoice and now
				// reset the deposits.
			...
			if err != nil {
				err = fmt.Errorf("unable to transition "+
		...
		case update := <-invoiceUpdateChan:
			...
			case invoices.ContractCanceled:
				err = fmt.Errorf("invoice for swap hash %v "+
					"canceled", f.loopIn.SwapHash)
...
func (f *FSM) SweepHtlcTimeoutAction(_ fsm.EventContext) fsm.EventType {
	for {
		err := f.createAndPublishHtlcTimeoutSweepTx()
		if err == nil {
			break
		}

		f.Errorf("unable to create and publish htlc timeout sweep "+
			"tx. Retrying in  %v", err)

		f.Errorf("retrying in %v", time.Hour.String())

		select {
		case <-f.ctx.Done():
			f.Errorf(f.ctx.Err().Error())
...
func (f *FSM) MonitorHtlcTimeoutSweepAction(_ fsm.EventContext) fsm.EventType {
	...
	htlcTimeoutTxidChan, errChan, err := f.cfg.ChainNotifier.RegisterConfirmationsNtfn( //nolint:lll
		f.ctx, f.loopIn.HtlcTimeoutSweepTxHash, timeoutSweepPkScript,
...
func (f *FSM) FetchSignPushSweeplessSweepTxAction(
	...
	fetchResp, err := f.cfg.Server.FetchSweeplessSweepTx(f.ctx, fetchReq)
	if err != nil {
	...
	sweeplessClientSigs, err := f.loopIn.signMusig2Tx(
		f.ctx, sweepTx, f.cfg.Signer, sessions, serverNonces,
	)
	if err != nil {
	...
	highFeeSigs, err := f.loopIn.signMusig2Tx(
		f.ctx, highFeeSweepTx, f.cfg.Signer, highFeeSessions,
		serverHighFeeNonces,
	)
	if err != nil {
	...
	extremelyHighSigs, err := f.loopIn.signMusig2Tx(
		f.ctx, extremelyHighFeeSweepTx, f.cfg.Signer, extremeSessions,
		serverExtremeNonces,
	)
	if err != nil {
	...
	_, err = f.cfg.Server.PushStaticAddressSweeplessSigs(f.ctx, req)
	if err != nil {
...
func (f *FSM) UnlockDepositsAction(_ fsm.EventContext) fsm.EventType {
	err := f.cfg.DepositManager.TransitionDeposits(
...
// sweep transaction.
func (f *FSM) createAndPublishHtlcTimeoutSweepTx() error {
	// Get a fee rate.
