...
// reactivateVerifiedDeposit refreshes an existing Deposited row with
...
func (m *Manager) reactivateVerifiedDeposit(ctx context.Context, d *Deposit,
	...
	*Deposit, bool, error) {
	...
	m.mu.Lock()
	_, wasActive := m.activeDeposits[verified.outPoint]
	m.mu.Unlock()
	...
	case Deposited:
		...
		// address linkage below. If the deposit is not currently active,
		// the recovery call restarts normal FSM tracking.
...
func (m *Manager) updateDepositConfirmations(ctx context.Context,
	...
	for _, utxo := range utxos {
		...
		var confirmationHeight int64
		switch {
		case utxo.Confirmations <= 0:
			confirmationHeight = 0

		case previousConfirmationHeight > 0:
			deposit.Unlock()
			continue
