...
func (f *FSM) updateDeposit(notification fsm.Notification) {
	...
	if isUpdateSkipped(notification, f.deposit) {
		return
	}
...
func isUpdateSkipped(notification fsm.Notification, d *Deposit) bool {
	...
	if prevState == Deposited && d.IsInState(Failed) {
		return true
	}
