...
func (m *Manager) recoverWithdrawals(ctx context.Context) error {
	...
	for txid, deposits := range depositsByWithdrawalTx {
		eg.Go(func() error {
			...
			// Spawn goroutine to monitor for spend/confirmation.
			// Best-effort: tx is published, caller continues.
			m.handleWithdrawal(
...
func (m *Manager) WithdrawDeposits(ctx context.Context,
	...
	// Persist state before starting the spend monitor. If monitoring setup
	// fails, recoverWithdrawals will find deposits in Withdrawing state on
	// restart and retry.
	...
	// allDeposited is true when deposits were in Deposited state (vs
	...
	if allDeposited {
		...
		// Get static address pkScript for spend registration.
		addrParams, err := m.cfg.AddressManager.GetStaticAddressParameters(ctx)
		if err == nil {
			// Spawn goroutine to monitor for spend/confirmation.
			// Best-effort: tx is published, caller continues.
...
func (m *Manager) handleWithdrawal(ctx context.Context,
	...
	go func() {
		...
		// Loop for spend registration retry and reorg recovery.
		for {
