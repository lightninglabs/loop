...
SELECT
    ...
    static_address_swaps.id, static_address_swaps.swap_hash, static_address_swaps.swap_invoice, static_address_swaps.last_hop, static_address_swaps.payment_timeout_seconds, static_address_swaps.quoted_swap_fee, static_address_swaps.deposit_outpoints, static_address_swaps.htlc_tx_fee_rate, static_address_swaps.htlc_timeout_sweep_tx_id, static_address_swaps.htlc_timeout_sweep_address,
...
const getStaticAddressLoopInSwaps = `-- name: GetStaticAddressLoopInSwaps :many
SELECT
    swaps.id, swaps.swap_hash, swaps.preimage, swaps.initiation_time, swaps.amount_requested, swaps.cltv_expiry, swaps.max_miner_fee, swaps.max_swap_fee, swaps.initiation_height, swaps.protocol_version, swaps.label,
    static_address_swaps.id, static_address_swaps.swap_hash, static_address_swaps.swap_invoice, static_address_swaps.last_hop, static_address_swaps.payment_timeout_seconds, static_address_swaps.quoted_swap_fee, static_address_swaps.deposit_outpoints, static_address_swaps.htlc_tx_fee_rate, static_address_swaps.htlc_timeout_sweep_tx_id, static_address_swaps.htlc_timeout_sweep_address,
    htlc_keys.swap_hash, htlc_keys.sender_script_pubkey, htlc_keys.receiver_script_pubkey, htlc_keys.sender_internal_pubkey, htlc_keys.receiver_internal_pubkey, htlc_keys.client_key_family, htlc_keys.client_key_index
...
type GetStaticAddressLoopInSwapsRow struct {
	ID                      int32
	SwapHash                []byte
	Preimage                []byte
	InitiationTime          time.Time
...
func (q *Queries) GetStaticAddressLoopInSwaps(ctx context.Context) ([]GetStaticAddressLoopInSwapsRow, error) {
	rows, err := q.db.QueryContext(ctx, getStaticAddressLoopInSwaps)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetStaticAddressLoopInSwapsRow
	for rows.Next() {
		var i GetStaticAddressLoopInSwapsRow
		if err := rows.Scan(
			&i.ID,
			&i.SwapHash,
			&i.Preimage,
...
func (q *Queries) UpdateStaticAddressLoopIn(ctx context.Context, arg UpdateStaticAddressLoopInParams) error {
	_, err := q.db.ExecContext(ctx, updateStaticAddressLoopIn, arg.SwapHash, arg.HtlcTxFeeRate, arg.HtlcTimeoutSweepTxID)
	return err
}
