...
// SweepHtlcRequest instructs loopd to sweep a swap HTLC.
type SweepHtlcRequest struct {
	...
	// Optional override for the stored swap preimage. This is required when
	// stateless_recovery is set.
	...
	// If set, reconstruct the latest Loop Out HTLC without querying the swap
	// database.
	StatelessRecovery *StatelessRecovery `protobuf:"bytes,7,opt,name=stateless_recovery,json=statelessRecovery,proto3" json:"stateless_recovery,omitempty"`
	// If set, request the server's MuSig2 signature and spend the HTLC via its
	// cooperative key path instead of revealing the preimage.
	Cooperative   *CooperativeSweep `protobuf:"bytes,8,opt,name=cooperative,proto3" json:"cooperative,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *SweepHtlcRequest) GetStatelessRecovery() *StatelessRecovery {
	if x != nil {
		return x.StatelessRecovery
	}
	return nil
}

func (x *SweepHtlcRequest) GetCooperative() *CooperativeSweep {
	if x != nil {
...
// SweepHtlcResponse returns the signed sweep transaction and publish outcome.
...
type NewStaticAddressResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The newly derived taproot static address.
	Address string `protobuf:"bytes,1,opt,name=address,proto3" json:"address,omitempty"`
	// The CSV expiry of the static address in blocks.
...
type FundStaticAddressRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Required. The funding request passed to lnd's SendCoins API. If addr is
	// empty, loopd creates and funds a fresh static address. If addr is set, it
	// must be an existing static address known to loopd. Set either a positive
	// amount or send_all, but not both.
	SendCoinsRequest *lnrpc.SendCoinsRequest `protobuf:"bytes,1,opt,name=send_coins_request,json=sendCoinsRequest,proto3" json:"send_coins_request,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *FundStaticAddressRequest) Reset() {
	*x = FundStaticAddressRequest{}
	mi := &file_client_proto_msgTypes[57]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FundStaticAddressRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FundStaticAddressRequest) ProtoMessage() {}

func (x *FundStaticAddressRequest) ProtoReflect() protoreflect.Message {
	mi := &file_client_proto_msgTypes[57]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use FundStaticAddressRequest.ProtoReflect.Descriptor instead.
func (*FundStaticAddressRequest) Descriptor() ([]byte, []int) {
	return file_client_proto_rawDescGZIP(), []int{57}
}

func (x *FundStaticAddressRequest) GetSendCoinsRequest() *lnrpc.SendCoinsRequest {
	if x != nil {
...
type FundStaticAddressResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The new or existing taproot static address receiving the funds.
	Address string `protobuf:"bytes,1,opt,name=address,proto3" json:"address,omitempty"`
	// The CSV expiry of the static address in blocks.
	Expiry uint32 `protobuf:"varint,2,opt,name=expiry,proto3" json:"expiry,omitempty"`
	// The response from lnd's SendCoins API for the funding transaction.
...
func (x *FundStaticAddressResponse) Reset() {
	*x = FundStaticAddressResponse{}
	mi := &file_client_proto_msgTypes[58]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FundStaticAddressResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FundStaticAddressResponse) ProtoMessage() {}

func (x *FundStaticAddressResponse) ProtoReflect() protoreflect.Message {
	mi := &file_client_proto_msgTypes[58]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use FundStaticAddressResponse.ProtoReflect.Descriptor instead.
func (*FundStaticAddressResponse) Descriptor() ([]byte, []int) {
	return file_client_proto_rawDescGZIP(), []int{58}
}

func (x *FundStaticAddressResponse) GetAddress() string {
	if x != nil {
		return x.Address
	}
	return ""
...
func (x *FundStaticAddressResponse) GetExpiry() uint32 {
	if x != nil {
		return x.Expiry
	}
	return 0
...
func (x *FundStaticAddressResponse) GetSendCoinsResponse() *lnrpc.SendCoinsResponse {
	if x != nil {
...
type StaticAddressSummaryResponse struct {
	...
	// The label for the legacy/root address in static_address, not for the
	// aggregate deposits or the most recently created receive address. Empty
...
// StatelessRecovery contains the public swap data needed to reconstruct the
// latest Loop Out HTLC without its database record.
type StatelessRecovery struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The server's compressed HTLC public key.
	ServerPubkey []byte `protobuf:"bytes,1,opt,name=server_pubkey,json=serverPubkey,proto3" json:"server_pubkey,omitempty"`
	// The client's compressed HTLC public key. The connected lnd wallet must
	// be able to derive the corresponding private key.
	ClientPubkey []byte `protobuf:"bytes,2,opt,name=client_pubkey,json=clientPubkey,proto3" json:"client_pubkey,omitempty"`
	// The absolute CLTV expiry committed to by the Loop Out HTLC.
	CltvExpiry int32 `protobuf:"varint,3,opt,name=cltv_expiry,json=cltvExpiry,proto3" json:"cltv_expiry,omitempty"`
	// The block height at which the swap was initiated. This is used as the
	// earliest possible confirmation height when locating the HTLC funding
	// transaction.
	SwapInitiationHeight int32 `protobuf:"varint,4,opt,name=swap_initiation_height,json=swapInitiationHeight,proto3" json:"swap_initiation_height,omitempty"`
	// The maximum number of client keys to scan in Loop's lnd key family if
	// signing by public key fails. If zero, loopd uses its default limit.
	KeyScanLimit  uint32 `protobuf:"varint,5,opt,name=key_scan_limit,json=keyScanLimit,proto3" json:"key_scan_limit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StatelessRecovery) Reset() {
	*x = StatelessRecovery{}
...
func (x *StatelessRecovery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StatelessRecovery) ProtoMessage() {}

func (x *StatelessRecovery) ProtoReflect() protoreflect.Message {
	mi := &file_client_proto_msgTypes[83]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use StatelessRecovery.ProtoReflect.Descriptor instead.
func (*StatelessRecovery) Descriptor() ([]byte, []int) {
	return file_client_proto_rawDescGZIP(), []int{83}
}

func (x *StatelessRecovery) GetServerPubkey() []byte {
	if x != nil {
...
func (x *StatelessRecovery) GetClientPubkey() []byte {
	if x != nil {
...
func (x *StatelessRecovery) GetCltvExpiry() int32 {
	if x != nil {
		return x.CltvExpiry
	}
	return 0
...
func (x *StatelessRecovery) GetSwapInitiationHeight() int32 {
	if x != nil {
		return x.SwapInitiationHeight
	}
	return 0
}

func (x *StatelessRecovery) GetKeyScanLimit() uint32 {
	if x != nil {
		return x.KeyScanLimit
	}
	return 0
}

// CooperativeSweep selects a cooperative MuSig2 key-path spend.
type CooperativeSweep struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The swap invoice payment address used to prove ownership to the server.
	// Stateful mode obtains this value from the stored swap invoice. Stateless
	// mode requires it here.
	PaymentAddress []byte `protobuf:"bytes,1,opt,name=payment_address,json=paymentAddress,proto3" json:"payment_address,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *CooperativeSweep) Reset() {
	*x = CooperativeSweep{}
...
func (x *CooperativeSweep) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CooperativeSweep) ProtoMessage() {}

func (x *CooperativeSweep) ProtoReflect() protoreflect.Message {
	mi := &file_client_proto_msgTypes[84]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use CooperativeSweep.ProtoReflect.Descriptor instead.
func (*CooperativeSweep) Descriptor() ([]byte, []int) {
	return file_client_proto_rawDescGZIP(), []int{84}
}

func (x *CooperativeSweep) GetPaymentAddress() []byte {
	if x != nil {
...
const file_client_proto_rawDesc = "" +
	...
	"\vsweep_tx_id\x18\x05 \x01(\tR\tsweepTxId\"h\n" +
	"\x17NewStaticAddressRequest\x12\x1d\n" +
	"\n" +
	"client_key\x18\x01 \x01(\fR\tclientKey\x12\x14\n" +
	"\x05label\x18\x03 \x01(\tR\x05labelJ\x04\b\x02\x10\x03R\x12send_coins_request\"}\n" +
	"\x18NewStaticAddressResponse\x12\x18\n" +
	"\aaddress\x18\x01 \x01(\tR\aaddress\x12\x16\n" +
	"\x06expiry\x18\x02 \x01(\rR\x06expiry\x12\x14\n" +
	"\x05label\x18\x04 \x01(\tR\x05labelJ\x04\b\x03\x10\x04R\x13send_coins_response\"a\n" +
	"\x18FundStaticAddressRequest\x12E\n" +
	"\x12send_coins_request\x18\x01 \x01(\v2\x17.lnrpc.SendCoinsRequestR\x10sendCoinsRequest\"\x97\x01\n" +
	"\x19FundStaticAddressResponse\x12\x18\n" +
	"\aaddress\x18\x01 \x01(\tR\aaddress\x12\x16\n" +
	"\x06expiry\x18\x02 \x01(\rR\x06expiry\x12H\n" +
	"\x13send_coins_response\x18\x03 \x01(\v2\x18.lnrpc.SendCoinsResponseR\x11sendCoinsResponse\"^\n" +
	...
	"\x1bStaticAddressSummaryRequest\"\xe4\x04\n" +
	...
	"\x12\x1e\n" +
	"\x1aFAILED_STATIC_ADDRESS_SWAP\x10\v2\x97\x16\n" +
	"\n" +
	"SwapClient\x129\n" +
	...
	"\x10NewStaticAddress\x12 .looprpc.NewStaticAddressRequest\x1a!.looprpc.NewStaticAddressResponse\x12Z\n" +
	"\x11FundStaticAddress\x12!.looprpc.FundStaticAddressRequest\x1a\".looprpc.FundStaticAddressResponse\x12o\n" +
...
var file_client_proto_goTypes = []any{
	...
	(*FundStaticAddressRequest)(nil),            // 67: looprpc.FundStaticAddressRequest
	(*FundStaticAddressResponse)(nil),           // 68: looprpc.FundStaticAddressResponse
	(*UpdateStaticAddressLabelRequest)(nil),     // 69: looprpc.UpdateStaticAddressLabelRequest
	(*UpdateStaticAddressLabelResponse)(nil),    // 70: looprpc.UpdateStaticAddressLabelResponse
	(*ListUnspentDepositsRequest)(nil),          // 71: looprpc.ListUnspentDepositsRequest
	(*ListUnspentDepositsResponse)(nil),         // 72: looprpc.ListUnspentDepositsResponse
	(*Utxo)(nil),                                // 73: looprpc.Utxo
	...
	(*StatelessRecovery)(nil),                   // 93: looprpc.StatelessRecovery
	(*CooperativeSweep)(nil),                    // 94: looprpc.CooperativeSweep
	nil,                                         // 95: looprpc.LiquidityParameters.EasyAssetParamsEntry
	(*lnrpc.OpenChannelRequest)(nil),            // 96: lnrpc.OpenChannelRequest
	(*swapserverrpc.RouteHint)(nil),             // 97: looprpc.RouteHint
	(*lnrpc.SendCoinsRequest)(nil),              // 98: lnrpc.SendCoinsRequest
	(*lnrpc.SendCoinsResponse)(nil),             // 99: lnrpc.SendCoinsResponse
	(*lnrpc.OutPoint)(nil),                      // 100: lnrpc.OutPoint
}
var file_client_proto_depIdxs = []int32{
	96,  // 0: looprpc.StaticOpenChannelRequest.open_channel_request:type_name -> lnrpc.OpenChannelRequest
	...
	97,  // 4: looprpc.LoopInRequest.route_hints:type_name -> looprpc.RouteHint
	...
	93,  // 13: looprpc.SweepHtlcRequest.stateless_recovery:type_name -> looprpc.StatelessRecovery
	94,  // 14: looprpc.SweepHtlcRequest.cooperative:type_name -> looprpc.CooperativeSweep
	24,  // 15: looprpc.SweepHtlcResponse.not_requested:type_name -> looprpc.PublishNotRequested
	25,  // 16: looprpc.SweepHtlcResponse.published:type_name -> looprpc.PublishSucceeded
	26,  // 17: looprpc.SweepHtlcResponse.failed:type_name -> looprpc.PublishFailed
	97,  // 18: looprpc.QuoteRequest.loop_in_route_hints:type_name -> looprpc.RouteHint
	89,  // 19: looprpc.QuoteRequest.asset_info:type_name -> looprpc.AssetLoopOutRequest
	90,  // 20: looprpc.OutQuoteResponse.asset_rfq_info:type_name -> looprpc.AssetRfqInfo
	97,  // 21: looprpc.ProbeRequest.route_hints:type_name -> looprpc.RouteHint
	40,  // 22: looprpc.TokensResponse.tokens:type_name -> looprpc.L402Token
	41,  // 23: looprpc.GetInfoResponse.loop_out_stats:type_name -> looprpc.LoopStats
	41,  // 24: looprpc.GetInfoResponse.loop_in_stats:type_name -> looprpc.LoopStats
	47,  // 25: looprpc.LiquidityParameters.rules:type_name -> looprpc.LiquidityRule
	0,   // 26: looprpc.LiquidityParameters.account_addr_type:type_name -> looprpc.AddressType
	95,  // 27: looprpc.LiquidityParameters.easy_asset_params:type_name -> looprpc.LiquidityParameters.EasyAssetParamsEntry
	4,   // 28: looprpc.LiquidityParameters.loop_in_source:type_name -> looprpc.LoopInSource
	1,   // 29: looprpc.LiquidityRule.swap_type:type_name -> looprpc.SwapType
	5,   // 30: looprpc.LiquidityRule.type:type_name -> looprpc.LiquidityRuleType
	45,  // 31: looprpc.SetLiquidityParamsRequest.parameters:type_name -> looprpc.LiquidityParameters
	6,   // 32: looprpc.Disqualified.reason:type_name -> looprpc.AutoReason
	14,  // 33: looprpc.SuggestSwapsResponse.loop_out:type_name -> looprpc.LoopOutRequest
	15,  // 34: looprpc.SuggestSwapsResponse.loop_in:type_name -> looprpc.LoopInRequest
	87,  // 35: looprpc.SuggestSwapsResponse.static_loop_in:type_name -> looprpc.StaticAddressLoopInRequest
	51,  // 36: looprpc.SuggestSwapsResponse.disqualified:type_name -> looprpc.Disqualified
	57,  // 37: looprpc.ListReservationsResponse.reservations:type_name -> looprpc.ClientReservation
	64,  // 38: looprpc.ListInstantOutsResponse.swaps:type_name -> looprpc.InstantOut
	98,  // 39: looprpc.FundStaticAddressRequest.send_coins_request:type_name -> lnrpc.SendCoinsRequest
	99,  // 40: looprpc.FundStaticAddressResponse.send_coins_response:type_name -> lnrpc.SendCoinsResponse
	73,  // 41: looprpc.ListUnspentDepositsResponse.utxos:type_name -> looprpc.Utxo
	100, // 42: looprpc.WithdrawDepositsRequest.outpoints:type_name -> lnrpc.OutPoint
	7,   // 43: looprpc.ListStaticAddressDepositsRequest.state_filter:type_name -> looprpc.DepositState
	...
	7,   // 47: looprpc.Deposit.state:type_name -> looprpc.DepositState
	84,  // 48: looprpc.StaticAddressWithdrawal.deposits:type_name -> looprpc.Deposit
	8,   // 49: looprpc.StaticAddressLoopInSwap.state:type_name -> looprpc.StaticAddressLoopInSwapState
	84,  // 50: looprpc.StaticAddressLoopInSwap.deposits:type_name -> looprpc.Deposit
	97,  // 51: looprpc.StaticAddressLoopInRequest.route_hints:type_name -> looprpc.RouteHint
	...
	46,  // 55: looprpc.LiquidityParameters.EasyAssetParamsEntry.value:type_name -> looprpc.EasyAssetAutoloopParams
	14,  // 56: looprpc.SwapClient.LoopOut:input_type -> looprpc.LoopOutRequest
	15,  // 57: looprpc.SwapClient.LoopIn:input_type -> looprpc.LoopInRequest
	17,  // 58: looprpc.SwapClient.Monitor:input_type -> looprpc.MonitorRequest
	19,  // 59: looprpc.SwapClient.ListSwaps:input_type -> looprpc.ListSwapsRequest
	22,  // 60: looprpc.SwapClient.SweepHtlc:input_type -> looprpc.SweepHtlcRequest
	27,  // 61: looprpc.SwapClient.SwapInfo:input_type -> looprpc.SwapInfoRequest
	53,  // 62: looprpc.SwapClient.AbandonSwap:input_type -> looprpc.AbandonSwapRequest
	28,  // 63: looprpc.SwapClient.LoopOutTerms:input_type -> looprpc.TermsRequest
	31,  // 64: looprpc.SwapClient.LoopOutQuote:input_type -> looprpc.QuoteRequest
	28,  // 65: looprpc.SwapClient.GetLoopInTerms:input_type -> looprpc.TermsRequest
	31,  // 66: looprpc.SwapClient.GetLoopInQuote:input_type -> looprpc.QuoteRequest
	34,  // 67: looprpc.SwapClient.Probe:input_type -> looprpc.ProbeRequest
	36,  // 68: looprpc.SwapClient.GetL402Tokens:input_type -> looprpc.TokensRequest
	36,  // 69: looprpc.SwapClient.GetLsatTokens:input_type -> looprpc.TokensRequest
	38,  // 70: looprpc.SwapClient.FetchL402Token:input_type -> looprpc.FetchL402TokenRequest
	42,  // 71: looprpc.SwapClient.GetInfo:input_type -> looprpc.GetInfoRequest
	12,  // 72: looprpc.SwapClient.StopDaemon:input_type -> looprpc.StopDaemonRequest
	44,  // 73: looprpc.SwapClient.GetLiquidityParams:input_type -> looprpc.GetLiquidityParamsRequest
	48,  // 74: looprpc.SwapClient.SetLiquidityParams:input_type -> looprpc.SetLiquidityParamsRequest
	50,  // 75: looprpc.SwapClient.SuggestSwaps:input_type -> looprpc.SuggestSwapsRequest
	55,  // 76: looprpc.SwapClient.ListReservations:input_type -> looprpc.ListReservationsRequest
	58,  // 77: looprpc.SwapClient.InstantOut:input_type -> looprpc.InstantOutRequest
	60,  // 78: looprpc.SwapClient.InstantOutQuote:input_type -> looprpc.InstantOutQuoteRequest
	62,  // 79: looprpc.SwapClient.ListInstantOuts:input_type -> looprpc.ListInstantOutsRequest
	65,  // 80: looprpc.SwapClient.NewStaticAddress:input_type -> looprpc.NewStaticAddressRequest
	67,  // 81: looprpc.SwapClient.FundStaticAddress:input_type -> looprpc.FundStaticAddressRequest
	69,  // 82: looprpc.SwapClient.UpdateStaticAddressLabel:input_type -> looprpc.UpdateStaticAddressLabelRequest
	71,  // 83: looprpc.SwapClient.ListUnspentDeposits:input_type -> looprpc.ListUnspentDepositsRequest
	74,  // 84: looprpc.SwapClient.WithdrawDeposits:input_type -> looprpc.WithdrawDepositsRequest
	76,  // 85: looprpc.SwapClient.ListStaticAddressDeposits:input_type -> looprpc.ListStaticAddressDepositsRequest
	78,  // 86: looprpc.SwapClient.ListStaticAddressWithdrawals:input_type -> looprpc.ListStaticAddressWithdrawalRequest
	80,  // 87: looprpc.SwapClient.ListStaticAddressSwaps:input_type -> looprpc.ListStaticAddressSwapsRequest
	82,  // 88: looprpc.SwapClient.GetStaticAddressSummary:input_type -> looprpc.StaticAddressSummaryRequest
	87,  // 89: looprpc.SwapClient.StaticAddressLoopIn:input_type -> looprpc.StaticAddressLoopInRequest
	10,  // 90: looprpc.SwapClient.StaticOpenChannel:input_type -> looprpc.StaticOpenChannelRequest
	16,  // 91: looprpc.SwapClient.LoopOut:output_type -> looprpc.SwapResponse
	16,  // 92: looprpc.SwapClient.LoopIn:output_type -> looprpc.SwapResponse
	18,  // 93: looprpc.SwapClient.Monitor:output_type -> looprpc.SwapStatus
	21,  // 94: looprpc.SwapClient.ListSwaps:output_type -> looprpc.ListSwapsResponse
	23,  // 95: looprpc.SwapClient.SweepHtlc:output_type -> looprpc.SweepHtlcResponse
	18,  // 96: looprpc.SwapClient.SwapInfo:output_type -> looprpc.SwapStatus
	54,  // 97: looprpc.SwapClient.AbandonSwap:output_type -> looprpc.AbandonSwapResponse
	30,  // 98: looprpc.SwapClient.LoopOutTerms:output_type -> looprpc.OutTermsResponse
	33,  // 99: looprpc.SwapClient.LoopOutQuote:output_type -> looprpc.OutQuoteResponse
	29,  // 100: looprpc.SwapClient.GetLoopInTerms:output_type -> looprpc.InTermsResponse
	32,  // 101: looprpc.SwapClient.GetLoopInQuote:output_type -> looprpc.InQuoteResponse
	35,  // 102: looprpc.SwapClient.Probe:output_type -> looprpc.ProbeResponse
	37,  // 103: looprpc.SwapClient.GetL402Tokens:output_type -> looprpc.TokensResponse
	37,  // 104: looprpc.SwapClient.GetLsatTokens:output_type -> looprpc.TokensResponse
	39,  // 105: looprpc.SwapClient.FetchL402Token:output_type -> looprpc.FetchL402TokenResponse
	43,  // 106: looprpc.SwapClient.GetInfo:output_type -> looprpc.GetInfoResponse
	13,  // 107: looprpc.SwapClient.StopDaemon:output_type -> looprpc.StopDaemonResponse
	45,  // 108: looprpc.SwapClient.GetLiquidityParams:output_type -> looprpc.LiquidityParameters
	49,  // 109: looprpc.SwapClient.SetLiquidityParams:output_type -> looprpc.SetLiquidityParamsResponse
	52,  // 110: looprpc.SwapClient.SuggestSwaps:output_type -> looprpc.SuggestSwapsResponse
	56,  // 111: looprpc.SwapClient.ListReservations:output_type -> looprpc.ListReservationsResponse
	59,  // 112: looprpc.SwapClient.InstantOut:output_type -> looprpc.InstantOutResponse
	61,  // 113: looprpc.SwapClient.InstantOutQuote:output_type -> looprpc.InstantOutQuoteResponse
	63,  // 114: looprpc.SwapClient.ListInstantOuts:output_type -> looprpc.ListInstantOutsResponse
	66,  // 115: looprpc.SwapClient.NewStaticAddress:output_type -> looprpc.NewStaticAddressResponse
	68,  // 116: looprpc.SwapClient.FundStaticAddress:output_type -> looprpc.FundStaticAddressResponse
	70,  // 117: looprpc.SwapClient.UpdateStaticAddressLabel:output_type -> looprpc.UpdateStaticAddressLabelResponse
	72,  // 118: looprpc.SwapClient.ListUnspentDeposits:output_type -> looprpc.ListUnspentDepositsResponse
	75,  // 119: looprpc.SwapClient.WithdrawDeposits:output_type -> looprpc.WithdrawDepositsResponse
	77,  // 120: looprpc.SwapClient.ListStaticAddressDeposits:output_type -> looprpc.ListStaticAddressDepositsResponse
	79,  // 121: looprpc.SwapClient.ListStaticAddressWithdrawals:output_type -> looprpc.ListStaticAddressWithdrawalResponse
	81,  // 122: looprpc.SwapClient.ListStaticAddressSwaps:output_type -> looprpc.ListStaticAddressSwapsResponse
	83,  // 123: looprpc.SwapClient.GetStaticAddressSummary:output_type -> looprpc.StaticAddressSummaryResponse
	88,  // 124: looprpc.SwapClient.StaticAddressLoopIn:output_type -> looprpc.StaticAddressLoopInResponse
	11,  // 125: looprpc.SwapClient.StaticOpenChannel:output_type -> looprpc.StaticOpenChannelResponse
