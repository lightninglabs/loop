...
| `static-labels/` | `loop static new --label`, `loop static summary` (label display), `loop static listunspent` and alias `l` label display, `loop static updatelabel` (relabel to `ops`, same-label update by applying `ops` twice to the same static address because normal single-client static-address sessions have one address, clear label, reapply `treasury`), final `loop static summary` |
