...
| `static-labels/` | `loop static new --label` (fresh receive address), `loop static summary` (independent legacy/root label), `loop static listunspent` and alias `l` label display, `loop static updatelabel` (relabel to `ops`, repeat the same update, clear label, reapply `treasury`), final `loop static summary` |
